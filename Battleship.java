...
public class Battleship extends JFrame {
    ...
    private static JButton done =new JButton("Done");//closes options menu
    private static JFrame options=new JFrame("Options");//holds opts
    private static JLabel data,//used for stats menu
            title;//used for options menu
    private static JPanel opts,//used for options menu
    ...
    private static JMenuItem m,pvp;//menu items
    ...
    public Battleship()
    {
        ...
        if (dummy==3)
        {
            JOptionPane.showMessageDialog(null,"Since you don't want to input your name"
                    + "I'll call you Noname","",JOptionPane.INFORMATION_MESSAGE);
            user="Noname";
        }
    ...
    public void whoGoesFirst()
    {
        ...
        if
        ((!players[x].getUser().equals("CPU1"))||(!players[x].getUser().equals("CPU2")))
        ...
        for (i=0;i<11;i++)
        {
            for (j=0;j<11;j++)
            {
                if ((j!=0)&&(i!=0))
                {
                    ...
                    {
                        players[u].getBboard(i-1,j-1).addActionListener(new ButtonListener());
                    }
    ...
    private class RulesListener implements ActionListener
    {
        public void actionPerformed(ActionEvent e)
        {
            int r= JOptionPane.showConfirmDialog(null,"Rules of the game.\n" +
                    "There are two players who place ships on their field or the ships are placed automatically. Each player moves in turn, if a player \n"+
                    "thinks for a very long time or takes a shot on his field, then the right to move \n"
                    +"automatically passes to another player, the player who first drowns all\n"+
                    " the opponent’s ships wins", "OK?", JOptionPane.OK_CANCEL_OPTION);
        }
    }
    ...
    public class OptionsListener implements ActionListener
    {
        ...
        public void setup()
        {
            opts=new JPanel(new GridLayout(2,2));
